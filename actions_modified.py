...
def cmd_raw(action, ability_id, queued, select_unit_id, target_unit_id):
  print(action, ability_id, queued, select_unit_id, target_unit_id)
  """
  Added by @danielfirebanks
  Select a specific unit using the raw interface."""

  # TODO NEXT STEPS: change name of function to command_raw, search for ability ids (send adam email)

  raw_act = action.action_raw.unit_command

  #TODO SEARCH FOR COMPATIBILITY WITH MULTIPLE UNITS AS ORGINAL C++ CODE.
...
TYPES = Arguments.types(
    ...
    target_unit_id=ArgumentType.scalar(5000000000),  # Depends on current selection.
...
FUNCTION_TYPES = {
    ...
    #TODO Should we add the units that perform the action? (I have curretly done so)
    cmd_raw: [TYPES.queued, TYPES.select_unit_id, TYPES.target_unit_id],
